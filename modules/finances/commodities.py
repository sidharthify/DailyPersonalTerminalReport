...
def get_data(config):
    symbols = config.get('symbols', ['GC=F', 'SI=F', 'CL=F', 'BZ=F'])
    ...
    for symbol in symbols:
        try:
            ...
            name = friendly_names.get(symbol.upper(), symbol)
            if price is not None and prev_close is not None:
                change = price - prev_close
                pct_change = (change / prev_close) * 100
                sign = "+" if change >= 0 else ""
                results.append(f"{name}: {price:,.2f} {display_curr} ({sign}{change:,.2f} | {sign}{pct_change:.2f}%)")
            elif price is not None:
                results.append(f"{name}: {price:,.2f} {display_curr}")
            else:
